...
DEFAULT_CONFIG_PATH = Path("configs/td3_humanoid_fast_probe.json")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train TD3 on Gymnasium Humanoid-v5 with reproducible logging."
    )
    ...
    parser.add_argument("--target-noise-clip", type=float, default=None)
...
def apply_overrides(config: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for key, value in td3_overrides.items():
        if value is not None:
...
def create_or_resume_run_dir(config: dict[str, Any], args: argparse.Namespace) -> Path:
    ...
    config_label = args.config.stem if args.config is not None else "td3_humanoid"
...
def build_model(config: dict[str, Any], train_env, tensorboard_dir: Path) -> TD3:
    td3 = config["td3"]
    policy_kwargs = {
        "net_arch": {
            "pi": list(td3["policy_net_arch"]),
            "qf": list(td3["qf_net_arch"]),
        }
    }
    ...
    return TD3(
        policy=td3["policy"],
        env=train_env,
        learning_rate=make_schedule(td3["learning_rate"]),
