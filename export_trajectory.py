...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export a trained policy trajectory without rendering."
    )
...
def main() -> int:
    ...
    output_dir = args.output_dir or args.run_dir / "trajectories"
    ...
    try:
        ...
        for episode_index in range(args.episodes):
            ...
            trajectory_path = output_dir / (
                f"{args.run_dir.name}_seed{episode_seed}_{stamp}_episode{episode_index}.npz"
            )
            np.savez_compressed(
                trajectory_path,
                env_id=env_id,
            ...
            trajectory_files.append(str(trajectory_path))
    ...
    summary_path = output_dir / f"{args.run_dir.name}_seed{args.seed}_{stamp}_summary.json"
    write_json(
        summary_path,
        {
